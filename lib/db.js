...
// Store DB file locally or in /tmp for serverless environments (Vercel)
...
function ensureDBFile() {
  ...
  try {
    ...
    console.error('Error reading DB file, using fallback memory store:', err);
    if (!memoryStore) {
      memoryStore = JSON.parse(JSON.stringify(INITIAL_DATA));
    }
  }
...
export function updateSiteContent(newContent) {
  const db = ensureDBFile();
...
export function updateProduct(id, updated) {
  ...
  if (idx !== -1) {
    const priceNum = Number(updated.price) || db.products[idx].price;
    db.products[idx] = {
      ...db.products[idx],
      ...updated,
      price: priceNum,
      formattedPrice: updated.formattedPrice || `₦${priceNum.toLocaleString()}`
    };
    saveDB(db);
...
export function updateGalleryItem(id, updated) {
  const db = ensureDBFile();
